...
DEBUT ALGORITHME CALCUL_IS_STRICT
    ...
    // 5. Taxes Additionnelles (Sur Base Imposable)
    
    // 5.a CSS (3% ou 4%)
    SI Secteur == FINANCE ALORS
        Montant_CSS = Base_IS * 0.04
    SINON
        Montant_CSS = Base_IS * 0.03
    FIN SI

    // 5.b Taxe Spéciale (4%)
    SI Secteur IN [FINANCE, TELECOM, AUTO] ALORS
        Montant_Taxe_Conso = Base_IS * 0.04
    SINON
        Montant_Taxe_Conso = 0
    FIN SI

    // 6. Total
    TOTAL_A_PAYER = IS_Du + Montant_CSS + Montant_Taxe_Conso
